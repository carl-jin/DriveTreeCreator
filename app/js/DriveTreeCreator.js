...
class DriveTreeCreator {
    ...
    return new Promise(async res => {
      if (window.gapi) {
        if (!window.gapi.client.script || !window.gapi.client.drive) {
          throw new Error(
            "make sure gapi.client.script or gapi.client.drive has been set up, use gapi.load pls"
          );
        } else {
          this._event();
          res();
  ...
  _event() {
    ...
    window.gapi.auth2
      ...
      .isSignedIn.listen(isSignedIn =>
        this._emitEvent("signInStateChange", { state: isSignedIn })
      );
  ...
  start(cache = false) {
    ...
    return new Promise(async res => {
      ...
      let result = [];
      this.options.googleAPI.folderId
        .split(",")
        .map(id => result.push(this._returnSpecificFolder(dir, id)));
      res(result);
    });
  }
      ...
      if (files[key].parents) {
        ...
        if (files[parentId]) {
          ...
          if (relationKeyPath[parentId]) {
            relationKeyPath[parentId] = this.__findKeyPath(
              parentId,
              relationKeyPath
            );
            let parentObj = null;
    ...
    return new Promise(res => {
      window.gapi.client.drive.files
        .list({
          ...
          spaces: "drive",
          q: `'${
            this.options.googleAPI.owner
          }' in owners and trashed = ${this.options.inTrash.toString()}`,
        ...
        .then(result => {
          ...
          //  emit event
          this._emitEvent("loadProcess", {
            process: count,
            count: result.result.items.length
          });
          count++;
          if (result.result.nextPageToken) {
            res(
              this._getAllFilesUnderRootFolder(
   ...
   */
  getCurrentUser() {
    let userProfile = window.gapi.auth2
      .getAuthInstance()
      .currentUser.get()
      .getBasicProfile();
    return userProfile
